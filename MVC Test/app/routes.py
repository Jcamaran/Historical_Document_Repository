...
@app_views.route('/')
def index():
    """ Render the main index page which acts as a landing page with links to other functionalities and displays all documents. """
    documents = Document.query.all()  # Fetch all documents from your database
    return render_template('index.html', documents=documents)

@app_views.route('/display_image/<int:document_id>')
def display_image(document_id):
    document = Document.query.get_or_404(document_id)
    return send_from_directory(directory=current_app.config['UPLOAD_FOLDER'], filename=document.filepath)
